...
function renderNav() {
  ...
  if (can('jb_view_journal') || can('jb_approve_auslagen')) {
    ...
    if (has('jb_buchungen')) addItem('auswertung', 'Auswertung (EÜR / Doppik)', { onClick: showAuswertung });
    if (has('jb_buchungen')) addItem('geschaeftsjahr', 'Geschäftsjahr & Anfangsbestände', { onClick: showGeschaeftsjahr });
...
function rerender() {
  ...
  else if (c.name === 'auswertung') showAuswertung();
  else if (c.name === 'geschaeftsjahr') showGeschaeftsjahr();
  else if (c.name === 'kontenblatt') showKontenblatt(c.konto, c.jahr);
...
async function showKassenbericht(year) {
  ...
  view.append(el('p', { class: 'sub' }, 'Live vom Server berechnet. Umbuchungen zwischen euren Konten zählen nicht als Einnahme oder Ausgabe.'));
  ...
  if ((data.topfe || []).length) {
    ...
    for (const p of data.topfe) pt.append(el('div', { class: 'tile' }, el('div', { class: 'tile-v' }, eur(p.saldo)), el('div', { class: 'tile-l' }, `${p.label} · Stand 31.12.${data.year}`)));
    view.append(pt);
  }
  ...
  if (d) {
    ...
    if ((d.konten || []).length) {
      for (const k of d.konten) line(`${k.name || k.konto} (${k.konto})`, k.saldo);
    } else {
    ...
    view.append(el('p', { class: 'muted' }, 'Kontostand je Geldkonto = Anfangsbestand + alle Zu- und Abgänge. Anfangsbestände unter Kassier:in → Geschäftsjahr.'));
...
async function showZbon() {
  ...
  function drawPreview() {
    ...
    const t = el('table');
    t.append(el('thead', {}, el('tr', {}, el('th', {}, 'Buchung'), el('th', {}, 'SKR-Konto'), el('th', {}, 'Geldkonto'), el('th', { style: 'text-align:right' }, 'Betrag'))));
    const tb = el('tbody');
    let sBar = 0;
    let sPP = 0;
    ...
    t.append(tb);
    prev.append(t);
    prev.append(el('p', { class: 'muted' }, `→ Barkasse ${eur(sBar)} (soll: ${eur(num(bar))}) · PayPal / Zettle ${eur(sPP)} (soll: ${eur(num(karte))})`));
...
/* ------------------------------------------------------------- Formular-Helfer */
/* Buchhaltungs-Ansichten (Journal, Auswertung, Geschäftsjahr …): buchhaltung.js */
